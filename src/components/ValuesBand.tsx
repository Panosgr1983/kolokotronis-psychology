      ...
      <div className="container-page py-16 md:py-20 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        {values.map((v) => (
          <div key={v.title} className="flex gap-4">
            <span className="shrink-0 size-11 rounded-full border border-footer-foreground/25 flex items-center justify-center text-cta">
              <v.icon className="size-5" strokeWidth={1.5} />
            </span>
            <div>
              <h3 className="font-serif text-lg mb-2">{v.title}</h3>
              <p className="text-sm text-footer-foreground/75 leading-relaxed">{v.text}</p>
            </div>
          </div>
        ))}
      </div>
